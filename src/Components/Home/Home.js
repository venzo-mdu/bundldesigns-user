...
// }, []);
  ...
  // ADD THIS SINGLE useEffect to replace all the above
useEffect(() => {
    ...
    if (hash === '#ourBundl') {
      const timeoutId = setTimeout(() => {
        ...
        if (element) {
          ...
          const targetPosition = Math.max(0, absoluteTop - 50);
      ...
      }, loading ? 1200 : 500); // Wait longer if still loading
...
}, [loading]); // Include loading as dependency
