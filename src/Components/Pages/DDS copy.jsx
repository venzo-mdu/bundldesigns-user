...
function OrderAddons({
  ...
  const [transitioningCard, setTransitioningCard] = useState(null); // e.g., `${item.id}_${filterIndex}`
          ...
          Array.from({ length: item.qty }, (_, qtyIndex) => qtyIndex + 1)
            ...
            .map((filterIndex) => {
              ...
              return (
                ...
                // >
                <div
                  key={`${item.id}_${filterIndex}`} // make sure each card has a unique key
                  style={{
                    transition: "opacity 0.5s, transform 0.5s",
                    opacity:
                    ...
                    <p className="my-6">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setTransitioningCard(cardKey);
                          setTimeout(() => {
                            saveContent(item.id, filterIndex, item.item__id);
                            addOnhandleRemove(filterIndex);
                            setTransitioningCard(null); // reset after transition
