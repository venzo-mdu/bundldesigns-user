...
export default function Dashboard({ lang, setLang }) {
  ...
  const [selectedText, setSelectedText] = useState(projects?.[0]?.project_name || "");
...
<div className="xs:px-[5%] flex items-center w-full relative">
   <span
        ref={spanRef}
        style={{
          position: "absolute",
  ...
  style={{
    fontSize: "16px",
    fontFamily: "inherit",
    border: "1px solid gray",  // <-- added border here
    borderRadius: "4px",       // optional, for nicer corners
    padding: "4px 8px",        // optional, some padding for better look
  }}
>
  {projects.map((proj) => (
