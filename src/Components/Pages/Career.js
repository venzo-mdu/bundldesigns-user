...
export default function Career({ lang, setLang }) {
  ...
  const validate = () => {
    ...
    if (!formData.message)
      newErrors.message =
        lang === "ar" ? "الوصف مطلوب" : "Description is required";

    if (!formData.vacancy)
      newErrors.vacancy =
        lang === "ar" ? "يرجى اختيار وظيفة شاغرة" : "Please select a vacancy";

    if (!formData.file)
      newErrors.file = lang === "ar" ? "يرجى رفع ملف" : "Please upload a file";
    else if (formData.file.size > 5 * 1024 * 1024)
      newErrors.file =
        lang === "ar"
          ? "ميغابايت 5 يجب أن يكون حجم الملف أقل من "
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      if (response.data) {
        ...
        setSuccessMsg(
          lang === "ar"
            ? "تم تقديم الطلب تم الإرسال بنجاح"
      ...
      <div className="font-Helvetica">
        <div className="text-center py-2 border-b border-black">
          <h1 className="lg:text-[38px] md:text-[38px] xs:text-[28px] lg:mt-[2%] md:mt-[2%] xs:mt-[5%] uppercase">
            {lang === "ar" ? "الوظائف المتاحة " : "Jobs"}{" "}
          </h1>
          <p className="lg:text-[18px] md:text-[18px] xs:text-[16px] text-[#00000080]">
            {lang === "ar"
              ? "حيث نجيب على جميع أسئلتك !"
        ...
        <div className="md:p-20 sm:p-3  border-b border-black">
          {vacancies.map((vacancy, index) => {
            return (
              <div
                className={`${
                  index + 1 != vacancies.length && "border-b"
                } border-black mb-6 px-6 `}
              >
                <p>
                  {" "}
                  <img
                    width="45px"
                    height="45px"
                    src={vacancy.image ? vacancy.image : careerImg}
                  ></img>
                </p>
                <h2 className="lg:text-[30px] md:text-[30px] xs:text-[20px] text-[#000] flex items-center">
                  {lang === "ar"
                  ...
                  >
                    <img
                      src={expandedVacancies[vacancy.id] ? upArrow : downArrow}
                    ></img>
                  </button>
                </h2>
        ...
        <div className="mt-24  mb-20">
          <h2 className="lg:text-[30px] md:text-[30px] xs:text-[20px] text-[#000]  mb-2 text-center">
            {lang === "ar"
              ? "تحب تنضم الى أسرة بندل؟ ارسلك سيرتك وملف اعمالك "
              : "See something you like? send us your CV & Recent Work"}
          </h2>
          <h3 className="text-[24px] mb-1 mt-4 text-center text-[#1BA56F]">
            {lang === "ar" ? "تواصل معنا !" : "Join Us!"}
          </h3>
          <form
            onSubmit={handleSubmit}
            className="px-6 pb-6 pt-1 sm:max-w-[90vw] md:max-w-[50vw] mx-auto space-y-4"
          >
            {/* Name Field */}
            ...
            <div className="mt-3 mb-3">
              ...
              />
              {errors.phone && (
                <p className="text-[#D83D99] text-[14px] text-sm">
                  {errors.phone}
                </p>
              )}
            </div>
            ...
            <div className="mt-3 mb-1">
              <textarea
                name="message"
                placeholder={
                  lang === "ar" ? "أخبرنا بأفكارك" : "Tell us your Thoughts"
                }
            ...
            <div className="mt-1 mb-2">
              <select
                ...
                onChange={handleChange}
                className={`w-full !rounded-none border !border-black p-2 ${
                  formData.vacancy ? "text-black" : "text-[#00000080]"
                }`}
              >
                <option disabled value={""} selected>
                  {lang === "ar"
                    ? "اختر الوظيفة الشاغرة"
                    : " Choose the vacancy"}{" "}
                </option>
              ...
              {errors.vacancy && (
                <p className="text-[#D83D99] text-sm">{errors.vacancy}</p>
              )}
            </div>
            ...
            <div className="my-3">
              ...
              <label
                htmlFor="file"
                className="cursor-pointer border !border-black !border-dashed p-3 flex flex-col items-center"
              >
                <img
                  src={fileUploadIcon}
                  alt="Upload Icon"
                ...
                <p className="text-[20px] text-gray-500">
                  {lang === "ar" ? "أسقط ملفاتك هنا" : "Drop your files here"}
                </p>
                {formData.file && (
              ...
              {errors.file && (
                <p className="text-[#D83D99] text-sm">{errors.file}</p>
              )}
            </div>
            ...
            {successMsg && (
              <p className="bg-green-600 py-1 px-4 flex justify-between items-center rounded text-white">
                {successMsg}{" "}
                <CloseIcon
                  onClick={() => {
                    setSuccessMsg(null);
