...
export default function UploadContent({ lang, setLang }) {
  ...
  const [visibleAddOnCards, setVisibleAddOnCards] = useState(
    Array.isArray(order?.item_details?.addon_items)
      ? order?.item_details?.addon_items.map(() => true)
      : []
  );
  ...
  useEffect(() => {
    if (
      ...
      let res = order?.item_details?.addon_items.map(() => true);
      setVisibleAddOnCards(res);
    }
  }, [order?.item_details?.addon_items]);

  const addOnhandleRemove = (index) => {
    const updatedVisibility = [...visibleAddOnCards];
    updatedVisibility[index] = false;
    setVisibleAddOnCards(updatedVisibility);
    setTimeout(() => {
      const newItems = [...addOnItems];
      newItems.splice(index, 1);
      setAddOnItems(newItems);
      const newVisibility = [...visibleAddOnCards];
      newVisibility.splice(index, 1);
      setVisibleAddOnCards(newVisibility);
    }, 5000);
  };
  return loading ? (
    <Bgloader />
                        ...
                        .map((filterIndex) => {
                          ...
                          return (
                            <div
                              style={{
                                // transition: "opacity 0.5s, transform 0.5s",
                                // opacity: visibleAddOnCards?.[filterIndex] ? 1 : 0,
                                // transform: visibleAddOnCards?.[filterIndex]
                                //   ? "translateX(0)"
                                //   : "translateX(-100%)", // <-- right to left animation
                                // background: "lightblue",
                                // padding: "10px",
                                ...
                                transition: "opacity 0.5s, transform 0.5s",
                                opacity: visibleAddOnCards?.[filterIndex]
                                  ? 1
                                  : 0,
                                transform: visibleAddOnCards?.[filterIndex]
                                  ? "translateY(0)"
                                  : "translateY(100%)", // <-- bottom to top animation
                                background: "lightblue",
                                padding: "10px",
                                whiteSpace: "nowrap",
                              }}
