...
export default function Adjustments() {
    ...
    const handleFileUpload = async (e, id, index) => {
        const files = e.target.files;
        if (files.length) {
            const formData = new FormData();
            const uploadedFilesArray = [];
    
            // Set the first file as the current uploading file
            setCurrentFile(files[0]); // New line added here to track the file being uploaded
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                uploadedFilesArray.push({
                    name: file.name,
                    url: URL.createObjectURL(file),  // Create a local URL for image preview
                });
            }
    
            // Update the adjustment form state with the first file name
            setAdjustmentForm((prevState) => ({
                ...prevState,
                file_name: uploadedFilesArray[0]?.name || 'Uploaded Files',
            }));
            ...
            try {
                ...
                // Assuming the response returns file URLs
                setAdjustmentsData((prev) => ({
                    ...prev,
                    [id]: {
                        ...prev[id],
                        attachments: response.data.file_url,
                        file_name: uploadedFilesArray.map((file) => file.name).join(", "),
                    },
                }));
    
                // Add the uploaded files to the list of uploaded files
                setUploadedFiles((prevFiles) => [...prevFiles, ...uploadedFilesArray]);
    
                // Clear the current file after successful upload
                setCurrentFile(null);  // Clear after successful upload
    
            } catch (error) {
                console.error('Error uploading file:', error);
                setCurrentFile(null);  // Reset currentFile in case of an error
            }
        }
    };
        ...
        <>
            ...
            >
                <input
                    ...
                    hidden
                    name="file"
                    id={`file-${adjustment.id}`}
                    multiple  // This allows multiple file uploads
                    onChange={(e) => handleFileUpload(e, adjustment.id, index)} // Handle file change
                />
...
{uploadedFiles.length > 0 && (
    <div className="uploaded-files">
        {uploadedFiles.map((file, index) => {
            // Skip rendering the file that is currently being uploaded
            if (file.name === currentFile?.name) return null;

            return (
                <div key={index} className="uploaded-file">
                    <span>{file.name}</span>
                    {/* Add a remove button (or cross icon) */}
                    <button 
