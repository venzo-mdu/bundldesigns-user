...
function OrderAddons({
  ...
  const handleRemoveWithAnimation = (id, filterIndex) => {
    ...
    setTimeout(() => {
      setCards((prev) =>
        prev.filter((c) => `${c.id}_${c.filterIndex}` !== key)
      );
      addOnhandleRemove(id, filterIndex); // Still call the external remove
      setTransitioningCard(null);
      setMovingUpCard(null);
    }, 4000); // Only 400ms is enough for animation
  };

  const initialCards = order
    .filter(
      (item) =>
        !skipId?.includes(item.id) && item.status === "questionnaire required"
    )
    .flatMap((item) =>
      Array.from({ length: item.qty }, (_, qtyIndex) => ({
        id: item.id,
        filterIndex: qtyIndex + 1,
        item,
      })).filter((entry) => !item.uploaded_qty?.includes(entry.filterIndex))
    );
  ...
  return (
    <div>
      {cards.map(({ id, filterIndex, item }, index) => {
        const cardKey = `${item.id}_${filterIndex}`;
        const isTransitioning = transitioningCard === cardKey;
        const isMovingUp = movingUpCard === cardKey;
        ...
        const transformStyle = isTransitioning
          ...
          ? "translateY(-70px)"
          : "translateY(0)";
        const opacityStyle = isTransitioning ? 0 : 1;
        ...
        return (
          ...
          >
            <div
              className={`${
                item.qty < 1 ? "" : "border-b !border-black"
              } px-[5%] space-x-2 mt-[2%]`}
            ...
            >
              ...
              {designQuestions[item.item__id]?.language && (
                <p className="mt-2 mb-0">
                  <label
                    className={`${lang === "ar" ? "ml-6" : "mr-6"} font-[500]`}
                  >
                    <input
                      type="radio"
                      value="English"
                      checked={
                        uploadContent[item.id]?.[filterIndex]?.language ===
                        "English"
                      }
                      onChange={(e) =>
                        handleChange(
              ...
              {designQuestions[item.item__id]?.measurement && (
                <>
                  ...
                  <p className="ml-2 mt-2">
                    ...
                    {uploadContent[item.id]?.[filterIndex]?.measurements ===
                      "Customize" && (
                      <>
                        <label className="text-[#1BA56F] mr-2">
                          ...
                          <input
                            ...
                            value={
                              uploadContent?.[item?.id]?.[filterIndex]?.width ||
                              ""
                            }
              ...
              {designQuestions[item.item__id]?.attachment && (
                <>
                  ...
                  >
                    ...
                    {uploadContent?.[item?.id]?.[filterIndex]?.filename ||
                      (lang === "ar" ? "تحميل المحتوى" : "Upload Content")}
                  </p>
                </>
              )}
