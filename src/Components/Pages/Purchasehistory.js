        ...
        window.innerWidth > 768 ? (
          purchases.length > 0 ? (
                  ...
                  <tr className='!mb-4'>
                    {Object.keys(dashboardJson.table_heads).map((key) => (
                      <th className='text-[#00000080] pb-2 lg:text-[20px] md:text-[16px] font-Helvetica font-medium'>
                        {lang === 'ar' ? dashboardJson.table_heads_arabic[key] : dashboardJson.table_heads[key]}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {purchases.map((project, index) => (
                    <tr key={project.id}>
                      ...
                      <td className={`lg:text-[20px] font-medium md:text-[16px] pb-2 text-[#1BA56F] ${index !== purchases.length - 1 ? 'border-b !border-[#00000080]' : ''}`}>{lang === 'ar' ? 'مكتمل' : 'Completed'}</td>
            ...
            <div className='relative py-10 pb-0'>
              <img className={`absolute ${lang === 'ar' ? 'sm:right-12 right-12 xs:right-[-3rem]' : 'sm:left-12 left-12 xs:left-[-3rem]'} sm:w-[200px] w-[200px] xs:w-[125px]`} style={{ transform: 'rotate(350deg)' }} src={workOurGIF} />
              <div className='w-[48%] text-center mx-auto'>
                ...
                <p className='flex justify-center mb-0 mt-0'><img className='animate-rotate-animation' width='150px' height='110px' src={workBrandGIF} /></p>
                <h2 className='lg:text-[32px] text-black mt-6'>{lang === 'ar' ? 'جاهز تبدأ رحلتك الابداعية معنا؟' : 'Inspired to start your journey to launch your next big thing?'}</h2>
                <p className='text-center'>
                  <button onClick={() => window.location.href = '/'} className='py-1 px-3 mt-[50px] border-black border-[1px] mb-8 bg-white hover:bg-black text-black uppercase hover:text-white'>
                    {lang === 'ar' ? 'ابدأ مشروعك الآن' : 'Get started !'}
                  </button>
                </p>
              </div>
              <img width='300px' className={`absolute sm:w-[300px] w-[300px] xs:w-[150px] xs:top-[30%] sm:top-[14%] ${lang === 'ar' ? 'xs:left-[-14%] sm:left-[3%] left-[3%]' : 'xs:right-[-14%] sm:right-[3%] right-[3%]'}`} style={{ transform: 'rotate(320deg)' }} src={workOurGIF} />
            </div>
          )
        ) : (
          purchases.length > 0 ? (
            <div className="w-full px-[8%] my-[10%]">
              <div className="flex justify-between items-center">
                ...
                <p className="underline text-[20px] font-[500] font-Helvetica text-[#1BA56F] cursor-pointer" onClick={() => setShowFull(!showFull)}>
              ...
              <div className={`transition-all duration-500 ease-out ${showFull ? "h-auto" : "h-[165px] overflow-hidden relative"}`}>
                {purchases.map((order, index) => (
                  <div key={index} className="border-b border-gray-300 py-2 flex flex-col md:flex-row md:items-center justify-between">
