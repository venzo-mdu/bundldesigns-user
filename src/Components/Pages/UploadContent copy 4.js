...
export default function UploadContent({ lang, setLang }) {
  ...
  const [items, setItems] = useState(order?.item_details?.bundle_items || []);

  const [visibleCards, setVisibleCards] = useState(
    Array.isArray(order?.item_details?.bundle_items)
      ? order?.item_details?.bundle_items.map(() => true)
      : []
  );
  ...
  useEffect(() => {
    if (
      ...
      let res = order?.item_details?.bundle_items.map(() => true);
      setVisibleCards(res);
    }
  }, [order?.item_details?.bundle_items]);

  const handleRemove = (index) => {
    // Step 1: Clone visibility array
    const updatedVisibility = [...visibleCards];

    // Step 2: Trigger in next frame to allow transition to apply
    requestAnimationFrame(() => {
      updatedVisibility[index] = false;
      setVisibleCards(updatedVisibility);

      // Step 3: After transition completes (2s), remove from state
      setTimeout(() => {
  ...
  const [addOnvisibleCards, setAddOnVisibleCards] = useState({});

  useEffect(() => {
    const initialVisibility = {};
    order?.item_details?.addon_items?.forEach((item) => {
      for (let i = 1; i <= item.qty; i++) {
        const key = `${item.id}-${i}`;
        initialVisibility[key] = true;
      }
    });
    setAddOnVisibleCards(initialVisibility);
  }, [order?.item_details?.addon_items]);

  // const addOnHandleRemove = (index) => {
  //   // Step 1: Fade out the card
  //   setAddOnVisibleCards((prev) => {
  //     const updated = [...prev];
  //     updated[index] = false;
  //     return updated;
  //   });

  //   // Step 2: Wait for the animation to complete before removing from list
  //   setTimeout(() => {
  ...
  //     setAddOnVisibleCards((prevVisibility) => {
  //       const updatedVisibility = [...prevVisibility];
  //       updatedVisibility.splice(index, 1);
  //       return updatedVisibility;
  //     });
  //   }, 2000); // Make sure this matches your transition duration
  // };

  // const addOnHandleRemove = (key) => {
  //   setAddOnVisibleCards((prev) => ({
  //     ...prev,
  //     [key]: false,
  //   }));
  ...
  //   setTimeout(() => {
  //     // Extract item.id from key (assumes format: `${id}-${filterIndex}`)
                          ...
                          return (
                            <div
                              style={{
                                ...
                                opacity: visibleCards?.[filterIndex] ? 1 : 0,
                                transform: visibleCards?.[filterIndex]
                                  ? "translateX(0)"
                                  : "translateX(-100%)", // <-- right to left animation
                    ...
                    .map((item, index, filteredArr) =>
                      Array.from(
                        ...
                        .map((filterIndex) => {
                          ...
                          const hasMultipleQty = item.qty < 1;
                          const uniqueKey = `${item.id}-${filterIndex + 1}`;
                          //  const uniqueKey = `${item.id}-${filterIndex + 1}`;
                          return (
                            <div
                              style={{
                                ...
                                opacity: addOnvisibleCards[uniqueKey] ? 1 : 0,
                                transform: addOnvisibleCards[uniqueKey]
                                  ? "translateX(0)"
                                  : "translateX(-100%)",
                                background: "lightblue",
                                padding: "10px",
