...
export const BundlDetail = ({ user, lang, setLang }) => {
  ...
  const getBundlData = async () => {
    ...
    let f = await axios.get(`${base_url}/api/order/cart/`, ConfigToken());

    // odification

    response.data.bundle_details.forEach((ele) => {
      f?.data?.item_details?.addon_items?.find((item) => {
        ele.design_list.forEach((designList) => {
          if (item.item__id === designList.id) {
            console.log(ele);
            // debugger;
            designList.quantity = item.qty;
          }
        });
      });
    });
