...
export const CustomBundl = ({user,lang,setLang}) => {
  ...
  const createPayload = async() => {
    ...
    const payload = isFromLogin ? savedPayload : {
  ...
  return (
    <div>
      ...
      <div className='bundl-detail mt-3'>
        ...
        <h2>{location?.state?.title || (lang === 'ar' ? 'صمم البندل الخاص بك' : 'Customize Bundl') }</h2>
          {/* <p className='bundl-desc-title'>Main outcomes: Brand Identity, Commerce Collateral, Social Media Starter Kit.</p> */}
          <p className='bundl-desc'>{lang === 'ar' ? 'المحتويات الأساسية: الهوية البصرية، عناصر التجار، مجموعة السوشال ميديا' : 'In this bundl, you have the freedom to mix and match from different add-ons that have been carefully curated to guarantee you find all the items needed for the success of your project.'}</p>
          <p className='one-minor mt-3'>{lang === 'ar' ? 'هذي الباقة تشمل تعديل مجاني واحد' : '* This Bundl includes one minor revision'}</p>
        </div>
          ...
          <div className='brand-details'>
           ...
           />
            ...
            <div style={{ margin: '5% 0 0 0' }}>
              <Accordian
                accordianTitle={lang === 'ar' ? 'صمم البندل الخاصة بك' : 'Customize your Bundl!'}
                textColor={'#1BA56F'}
          ...
          <div className='bundl-name'>
                        ...
                        xs:flex sm:block block flex-wrap justify-around`
                            ...
                            >
                              <div className='flex xs:w-[100%] w-full'>
                                ...
                                <p
                                  className={`sm:text-[18px] text-[18px] xs:text-[16px] font-[400] lg:w-[40%] md:w-[50%] xs:w-[25%]  ${lang === 'ar' ? 'text-left' : 'text-right'}`} 
                                >
            ...
            <div className='bundl-checkout sm:mt-3 xs:mt-0'>
              ...
              <div className='total  flex items-center' >
                ...
                <p className='w-[40%] xs:text-right sm:text-left sm:mb-2 xs:mb-0'>{ addonPayLoads.total_time} {lang === 'ar' ? 'يوم' : 'Days'}</p>
              </div>
